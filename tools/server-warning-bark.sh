...
server_name='rend'
#bark服务器url&key
barkserver='https://api.day.app/your_key/'
...
echo "CPU总核数为 $cpu_num"

#抓取当前系统15分钟的平均负载值
load_15=`uptime | awk '{print $12}'`
echo "系统15分钟的平均负载值为 $load_15"
...
curl "$barkserver$server_name 服务器系统负载严重告警！！！/$server_name服务器15分钟的系统平均负载为$average_load，超过警戒值1.0，请立即处理！！！" 
else
...
curl "$barkserver$server_name 服务器系统负载告警！！！/$server_name服务器15分钟的系统平均负载为$average_load，超过警戒值0.70，请立即处理！！！" 
fi
...
curl "$barkserver$server_name 服务器CPU告警！！！/$server_name服务器cpu剩余$cpu_idle%，使用率已经超过80%，请及时处理。" 
fi
...
echo "当前剩余的交换分区 $swap_free"

#当前已使用的交换分区used大小
swap_used=`free -m | grep Swap | awk '{print $3}'`
echo "当前已使用的交换分区为 $swap_used"
...
curl "$barkserver$server_name 服务器存储空间告警！/$server_name服务器swap交换分区只剩下 $swap_free M 未使用，剩余不足20%，请及时处理。"
fi
...
echo "当前根分区总容量为$disk_total，已用$disk_usage"

#设置空闲硬盘容量的告警值为80%，如果当前硬盘使用超过80%，立即发邮件告警
if [ "$disk_usage" -gt 80 ]; then
echo "$time 服务器硬盘空间已使用超过80%，注意！" > server_warning.log
curl "$barkserver$server_name 服务器存储空间告警！/$server_name 服务器 /根分区 使用率已经超过80%，请及时处理。" 
fi
...
curl "$barkserver$server_name 服务器安全告警！/$server_name 服务器用户数已经达到$users个，请及时处理。" 
...
if [ "$nginxnum" -le 1 ]; then
echo "$time 服务器紧急，nginx进程结束！" > server_warning.log
curl "$barkserver$server_name 服务器进程告警！/$server_name服务器紧急，nginx进程结束！" 
fi
fi
...
curl "$barkserver$server_name 服务器进程告警！/$server_name服务器紧急，apache进程结束！" 
fi
